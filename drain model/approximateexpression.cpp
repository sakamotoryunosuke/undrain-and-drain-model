...
double Ep = 0.01;//dilatancy parameter
...
int main ()
{
	...
	char filename[128];
	ofstream file;
	sprintf(filename,"approximatesolution Ep_%.4f plate_%.0f.dat",Ep,plate);
	file.open(filename);
for(double T_d= pow(10.,-2.) ;T_d<=pow(10.,-1.);T_d+=pow(10.,-3.)){
...
double V_pld = 2*M_PI/T_d;// nondimensional pulling spring velocity
	...
	double Breal = std::real(ak-1./D*(bk-stemyu*Ep*sqrt(2*M_PI)*C));
	double Bimag = std::imag(ak-1./D*(bk-stemyu*Ep*sqrt(2*M_PI)*C));
	
	std::complex<double> BB(Breal,Bimag); //eq(S5)
